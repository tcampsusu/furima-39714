...
class OrderAddress
 ...
 with_options presence: true do
  ...
  validates :city
  validates :block
  validates :phone_no, length: { minimum: 10, maximum: 11,message: "is too short" },format: { with: /\A\d{10,11}\z/, message: "is invalid." }
  validates :item_shipping_area_id, numericality: { other_than: 1, message: "can't be blank" }
  validates :token
end
